...
Class ModelStoreorder extends CI_Model 
{
    ...
    public function order_process()
    {
        ...
        for($x = 0; $x < $count_product; $x++) {
            ...
            $product_data = $this->ModelProductstore->getProductData($this->input->post('product')[$x]);
            $qty = (int) $product_data['qty'] - (int) $this->input->post('qty')[$x];

            $update_product = array('qty' => $qty);


            $this->ModelProductstore->update($update_product, $this->input->post('product')[$x]);
        }
        if($order_id)
        {
            $net_amount = $this->input->post('net_amount_value');
    ...
    public function order_item_data($order_id = null)
    {
        ...
        $this->db->select( 'orders_item.*,warehouse_products.name' );
        $this->db->from( 'orders_item' );
        $this->db->join( 'warehouse_products','orders_item.product_id = warehouse_products.product_id','left' );
        $this->db->where( 'orders_item.order_id', $order_id);
        $query = $this->db->get();
        $result = $query->result_array();
        return $result;
    }
    public function search_order_data($data)
    {
        $user_id = $this->session->userdata('id');
        $this->db->from( 'orders' );
        if($data['bill_no'] != '')
        {
            $this->db->where( 'orders.bill_no', $data['bill_no']);   
        }
        if($data['customer_phone'] != '')
        {
            $this->db->where( 'orders.customer_phone', $data['customer_phone']);   
        }
        $this->db->where( 'orders.user_id', $user_id);
        $query = $this->db->get();
        $result = $query->result_array();
