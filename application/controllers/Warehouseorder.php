...
class Warehouseorder extends CI_Controller {
	public function __construct() 
	{
		...
		$this->load->model('ModelWarehouseorder');
		$this->load->model('ModelStore');
	}
	public function index()
	{
		$result['order'] = $this->ModelWarehouseorder->getOrdersData();
		/*echo "<pre>";
    	print_r($result);
    	echo "</pre>";
    	exit();*/
		$this->load->view('header');
		$this->load->view('manage_warehouse_order',$result);
		$this->load->view('footer');
	}
	public function addwarehouseorder()
	{
		$id = $this->uri->segment(3);
		$company = $this->ModelWarehouseorder->getwarehouseData($id);
    	$this->data['company_data'] = $company;
    	$this->data['is_service_enabled'] = ($company['service_charge_value'] > 0) ? true : false;
    	$this->data['products'] = $this->ModelAdminproduct->manage_product(); 
    	/*echo "<pre>";
    	print_r($this->data);
    	echo "</pre>";
    	exit();*/
		$this->load->view('header');
		$this->load->view('add_warehouse_order',$this->data);
		$this->load->view('footer');
	}
	public function fetch_product_qty()
	{
		$product_id = $this->input->post('product_id');
		$response_data = $this->ModelAdminproduct->edit_product($product_id);
		echo json_encode($response_data);
    }
	public function order_process()
	{
		$response_data = $this->ModelWarehouseorder->order_process();
	...
	public function warehose_bill_sms($phone,$bill,$name,$amount,$item)
	{
		...
		$details = "http://sms.pearlsms.com/public/sms/send?sender=DVYTRP&smstype=TRANS&numbers=".$phone."&apikey=b617ec3e3bc74a10a5d268ac41b10b01&message=Hello,%20".$name.",%20thank%20you%20for%20shopping%20with%20LA%20MONT%20perfumes.%20Here%27s%20your%20invoice%20for%20order%20".$bill."%20with%20".$item."%20items%20worth%20Rs%20".$amount."%20You%20can%20see%20invoice%20https://chiragkheni.com/assets/warehouse_bill/".$bill.".pdf%20Viste%20our%20instgram%20page%20https....%20DVYTRP";
	...
	public function getProductValueById()
	{
		$product_id = $this->input->post('product_id');
		if($product_id) {
			$product_data = $this->ModelAdminproduct->getProductData($product_id);
			echo json_encode($product_data);
		}
	}
	public function getTableProductRow()
	{
		$products = $this->ModelAdminproduct->getActiveProductData();
		echo json_encode($products);
	}
	function GeneratePdf($id)
	{
		$result['order_data'] = $this->ModelWarehouseorder->getOrdersData($id);
		$result['orders_items'] = $this->ModelWarehouseorder->order_item_data($id);
		$result['order_date'] = date('d/m/Y', $result['order_data']['date_time']);
		$result['paid_status'] = ($result['order_data']['paid_status'] == 1) ? "Paid" : "Unpaid";
		$this->load->view('warehouse_bill_pdf',$result);
	...
	{   
    	...
    	exit();*/
			...
			$this->load->view('header');
			$this->load->view('warehouse_invoice_print',$result);
			$this->load->view('footer');
